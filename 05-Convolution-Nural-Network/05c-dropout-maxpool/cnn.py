...
class BaselineCNN(nn.Module):
    def __init__(self, num_classes: int = 10):
        """
        Creates a baseline CNN without dropout or max-pooling layers.
...
class DropoutMaxPoolCNN(nn.Module):
    def __init__(self, num_classes: int = 10, dropout_p: float = 0.25):
        """
        Creates a CNN that uses max-pooling and dropout.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Calculates logits of the dropout+max-pool CNN.
...
def train_short_comparison(
    ...
    """
    Trains a baseline CNN and a dropout+max-pool CNN on a small CIFAR-10 subset.
    ...
    Args:
        ...
        weight_decay (float): Weight decay used by Adam.
    ...
    models = {
        "baseline": BaselineCNN(num_classes=10).to(device),
        "dropout+maxpool": DropoutMaxPoolCNN(num_classes=10).to(device),
    }
    histories = {}
    ...
    regularized_acc = histories["dropout+maxpool"]["test_acc"][-1]
    delta = regularized_acc - baseline_acc

    print(
        "\nFinal test accuracy: "
        f"baseline={100 * baseline_acc:.2f} %, "
        f"dropout+maxpool={100 * regularized_acc:.2f} %, "
