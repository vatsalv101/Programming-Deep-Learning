...
def _denormalize_images(images: torch.Tensor) -> torch.Tensor:
    images = images * FASHION_MNIST_STD + FASHION_MNIST_MEAN
...
def _image_grid(images: torch.Tensor, titles: list[str], title: str, cols: int = 4, top_margin: int = 80, vertical_spacing: float = 0.04):
    ...
    images = _denormalize_images(images).detach().cpu()
    images = (255 * images.permute(0, 2, 3, 1)).to(torch.uint8)
...
def fashion_mnist_pictures(n: int = 12):
    """
    Plots random images from the Fashion-MNIST test dataset.
    ...
    return _image_grid(images, titles, title="Fashion-MNIST Examples", cols=4, top_margin=60, vertical_spacing=0.1)
...
def show_training_comparison(histories):
    ...
    Args:
        histories (dict): Dictionary returned by train.train_short_comparison.
    ...
    fig.update_layout(title="Short MLP Comparison")
